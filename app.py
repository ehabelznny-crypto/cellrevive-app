...
# ==============================================================================
# 1️⃣ الإعدادات المتكاملة والمطابقة التامة للهواتف المحمولة والأجهزة الذكية
# ==============================================================================
st.set_page_config(
...
# هندسة الواجهة الفاخرة المحدثة بالكامل لعام 2026 بألوان بيضاء وصفراء واضحة جداً للموبايل
st.markdown("""
    <style>
    ...
    label, .stMarkdown, p, span, .stCheckbox {
        color: #ffffff !important;
...
# قاعدة بيانات الدليل المتكامل لعام 2026
EGYPTIAN_DRUG_DB = {
...
# إتاحة رفع واختيار حتى 15 نوع دواء متزامن لتدقيق الحالة بدقة بالغة
...
if uploaded_skin_img is not None:
    ...
    skin_findings = [
        ...
        "3. حب الشباب (Acne): رصد بثور متجمعة ومتركزة في منطقة الذقن والفك السفلي تومئ باضطراب أندروجينيأيضي مصاصب للمقاومة.",
    ...
    st.success("🔬 تم اكتمال التدقيق المجهري السداسي للجلد! تم توثيق ورصد العلامات الست بدقة متناهية باللون الأبيض والذهبي الجلي.")
else:
...
# القياسات الحيوية للعيادة والمطورة
...
# ==============================================================================
# 6️⃣ تشغيل المحرك المتكامل والمقاصة الدوائية وإصدار التقارير الكبرى لـ 15 دواء
# ==============================================================================
st.write("")
if st.button("🚀 تفعيل محرك الترميم الخلوي وإصدار التقرير المتكامل المعقد", use_container_width=True):
    st.markdown("---")
    
    # المعادلات الرياضية المتقدمة لربط الجلد بالدواء بالأيض والوحدات الحجمية للوجبة
    ...
    # معالجة القائمة الموسعة للأدوية الـ 15 المنتخبة في التقرير
    for selected_item in selected_drugs_list:
        for key, data in EGYPTIAN_DRUG_DB.items():
            if data["ar"] in selected_item:
                clinical_warnings.append(f"🚨 تنبيه المقاصة الحيوية المباشر لـ [{data['ar']}]: {data['risk']}")
    ...
    # عرض بطاقات المخرجات الفاخرة بالألوان الذهبية والأبيض القابلة للقراءة بامتياز
    st.markdown(f"""
    ...
    # لوحة التنبيهات الدوائية الصارمة المأخوذة من دليل 2026
    if clinical_warnings:
        st.markdown("<h4 style='color:#ffff00; font-weight:bold;'>⚠️ تدابير دليل الدواء المصري وتنبيهات المقاصة الحيوية:</h4>", unsafe_allow_html=True)
        for warn in clinical_warnings:
            st.warning(warn)

    # إخراج نتائج التطهير البصري والأيضي للوجبات بالوحدات المنزلية الصارمة
    st.markdown('<div class="premium-card"><h4 style="color:#d4af37; margin:0;">🥗 نتيجة تحليل وتطهير الطبق بالوحدات المنزلية الحركية المعتمدة</h4></div>', unsafe_allow_html=True)
    ...
    with col_d1:
        ...
        st.write("3. **الفلترة المعوية:** يوصى قسرياً بتناول **كوب كبير (240 مل) من الماء الدافئ المضاف إليه ملعقة صغيرة من خل التفاح العضوي** غير المصفى، مع بدء الوجبة بـ **طبق سلطة خضراء ضخم بحجم قبضة اليد المغلقة مرتين**؛ لتعطيل تفكيك نشويات الأرز معوياً.")
    with col_d2:
    ...
    =======================================================
    المنصة العالمية للترميم الخلوي وعكس مسار مقاومة الانسولين والسكري من النوع الثاني - إشراف د. إيهاب حشمت الظني
    ...
    - قائمة الأدوية النشطة المكتشفة والمفحوصة (حتى 15 دواء): {', '.join(selected_drugs_list) if selected_drugs_list else 'لا يوجد تداخلات حرجة مرصودة'}
    ...
    - حصة كمية البروتين الآمنة والمعدلة نسيجياً لحماية الفلترة: {round(safe_protein_grams_daily, 1)} جرام/يومياً كحد أقصى لحماية الفلترة الكلوية
    -------------------------------------------------------
    ...
    st.text_area("معاينة مستند التقرير الطبي المتكامل الموجه قبل التحميل والطباعة البصرية والورقية:", report_text, height=250)
