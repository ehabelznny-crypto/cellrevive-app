...
with col1:
    patient_name = st.text_input("اسم المريض بالكامل (اكتبه بالإنجليزية للتقرير الطبي):", value="Ahmed Mohamed")
...
if "Deltacortril" in medication:
    ...
    restricted_organ = "Kidney Protection"
elif "Ketosteril" in medication:
    max_allowed_protein_ratio = 0.6
    restricted_organ = "Kidney Cells Protection"
...
if st.button("🚀 تفعيل بروتوكول الترميم الخلوي وحساب البصمة"):
    ...
    * 🫘 **البقوليات والأكواب:** ما يعادل ({foul_cup}) كوباً (سعة 240 مل) من الفول أو العدس المطبوخ.
    ...
    # نص التقرير تم تنظيفه تماماً من أي حروف عربية أو إيموجي لمنع خطأ الـ Unicode
...
"""
    ...
    # كتابة النص داخل الـ PDF بأمان
    pdf.multi_cell(0, 6, report_content)
    
    # التصدير الآمن للبيانات
