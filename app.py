...
# ==============================================================================
# 3️⃣ محرك فحص الأدوية المتطور والذكي (الكتابة والصور اللامحدودة)
# ==============================================================================
st.markdown('<div class="premium-card"><h3 style="color:#d4af37; margin:0;">💊 مستشار المقاصة الدوائية الذكي والفحص البصري التكنولوجي</h3><p style="font-size:12.5px; color:#ffff00;">تحليل وفحص متناهي الدقة لكافة الأدوية المدخلة نصياً أو الملتقطة بصرياً عبر كاميرا الهاتف لضمان سلامة الميتوكوندريا وعزل وهن العضلات.</p></div>', unsafe_allow_html=True)
...
selected_drugs_list = st.multiselect(
    "💊 حدد الأدوية الشائعة التي يتناولها المريض حالياً (إن وجدت):",
    options=[f"{data['ar']} ({key.capitalize()})" for key, data in EGYPTIAN_DRUG_DB.items()]
)

custom_drugs_input = st.text_area(
    "✍️ اكتب هنا كافة الأدوية الأخرى، الروشتات، أو المكملات التي يتناولها المريض بكل دقة ونوعها (دواء دواء):",
    placeholder="مثال: نيرفيزام قرص مساءً، أميريل 2 مجم صباحاً..."
)
...
if uploaded_skin_imgs:
    ...
    skin_findings = [
        ...
        "3. حب الشباب الهرموني (Acne): بثور متركزة في منطقة الذقن والفك السفلي تومئ باضطراب أندروجيني أيضي مصاحب للمقاومة.",
        "4. اسمرار الوجه الأيضي: تصبغات داكنة عشوائية ناتجة عن تنشيط الخلايا الصبغية بفعل المتلازمة الأيضية.",
        "5. جفاف وتصلب نسيج الجلد: تراجع التروية الدقيقة وبطء مرونة جدار البشرة الحجمية."
    ]
    clinical_severity_score = 5.5
else:
    ...
    if st.checkbox("🎯 الشواك الأسود (Acanthosis nigricans): بقع داكنة مخملية وسميكة في الرقبة، الإبطين"):
        skin_findings.append("⚠️ الشواك الأسود: بقع داكنة مخملية وسميكة في طيات الجسم تعكس فرط الإنسولين.")
        clinical_severity_score += 2.0
    if st.checkbox("🎯 الزوائد الجلدية (Skin tags): نمو زائد وصغير للجلد حول العينين أو الرقبة"):
    ...
    if st.checkbox("🎯 جفاف الجلد: زيادة ملحوظة في جفاف البشرة وتراجع مرونتها"):
...
if uploaded_meal_imgs:
    st.info(f"🥗 تم استقبال {len(uploaded_meal_imgs)} لقطات لمكونات الوجبة. يجري قياس حجوم النشويات والبروتينات هندسياً...")
    cols_meal = st.columns(min(len(uploaded_meal_imgs), 5))
    for idx, img in enumerate(uploaded_meal_imgs):
        with cols_meal[idx]:
...
if st.button("🚀 تفعيل محرك الترميم الخلوي وإصدار التقرير المتكامل المعقد", use_container_width=True):
    ...
    with col_res2:
        ...
        st.metric(label="🛡️ المحاور العضوية المتأثرة بقائمة الأدوية والمقاصة", value=", ".join(organs_at_risk) if organs_at_risk else "آمنة ومستقرة تماماً")

    # لوحة التنبيهات الدوائية الصارمة
    if clinical_warnings or custom_drugs_input:
        st.markdown("<h4 style='color:#ffff00; font-weight:bold; text-align:right;'>⚠️ تدابير دليل الدواء وملاحظات الفحص التكنولوجي:</h4>", unsafe_allow_html=True)
        for warn in clinical_warnings:
            st.warning(warn)
        if custom_drugs_input:
            st.info(f"📝 قائمة الأدوية الإضافية المفحوصة نصياً وبصرياً: {custom_drugs_input}")
    ...
    # ==============================================================================
    # 7️⃣ محرك بناء وتوليد التقرير الطبي الشامل وعرضه الآمن تماماً من أي خط طولي
    # ==============================================================================
    st.write("---")
    ...
    drugs_report_str = ", ".join(selected_drugs_list) if selected_drugs_list else ""
    if custom_drugs_input:
        drugs_report_str += f" | أدوية إضافية: {custom_drugs_input}"
    if not drugs_report_str:
        drugs_report_str = "لا يوجد تداخلات حرجة مرصودة"
    ...
    report_text = (
        ...
        "-------------------------------------------------------\n"
        "[بصمة فحص علامات الجلد المتعددة - محرك الرؤية الحاسوبية والذكاء الاصطناعي]:\n"
        f"{skin_report_str}"
        "-------------------------------------------------------\n"
        ...
        "-------------------------------------------------------\n"
        "[المقاصة الدوائية وتدابير دليل الدواء وفحص الصور والروشتات]:\n"
        f"- قائمة الأدوية المكتشفة والمفحوصة تقنياً: {drugs_report_str}\n"
        ...
        "-------------------------------------------------------\n"
        "التوصية الإكلينيكية الموجهة لسيادتكم: بناءً على نتائج الفحص المجهري المتعدد للجلد والمقاصة الدوائية لعام 2026 الصادرة عن دليل هيئة الدواء المصرية ووزارة الصحة وتحليل الصور الروشتية، يوصى بعزل النشويات وتعديل كمياتها بالوحدات القياسية (كف اليد، الملاعق، علبة الكبريت، الكوب 240 مل) وتعديل بروتوكول العلاج لحماية الميتوكوندريا والأعضاء المتأثرة دوائياً، ودعم خلايا المريض بالمغنيسيوم والمكملات الخلوية اللازمة لعكس مسار السكري ومتلازمة التمثيل الغذائي.\n"
        "-------------------------------------------------------\n"
